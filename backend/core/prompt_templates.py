...
- "dialogue"：只填本镜真正需要发声的内容，可包含真实对白、画外旁白、内心OS/吐槽；必须保留声源标记，如「对白：」「画外旁白：」「内心OS：」。不出声的心理描写不要放入 dialogue。不要把动作描写写进 dialogue。
...
严格输出 JSON 数组（不要额外文字），每个元素：
{
  ...
  "action": "动作/事件",
  "camera": "机位/景别/运动",
  "dialogue": "声音内容（对白/画外旁白/内心OS；无则空）",
  "emotion": "情绪",
...
- "dialogue"：只填本镜真正需要发声的内容，可包含真实对白、画外旁白、内心OS/吐槽；必须保留声源标记，如「对白：」「画外旁白：」「内心OS：」。无声音留空。
...
- 打斗、追逐、环绕、复杂推拉摇移、多人物调度、关键剧情反转：考虑导演图。
...
【生成约束】
禁止字幕、禁止水印；动作自然铺满总时长，保持参考图对象一致，避免穿模、多手多指、物体融化或比例漂移。"""
...
要求：
1) 引用统一用 @名，且只能引用【本镜关联资产】或【连续性桥接变量】明确存在的对象；不得自造新 @名。库内 #/$ 分类符号不得出现在输出里。
   - 普通资产：写 @角色、@场景、@道具。
   - 连续性素材：只有桥接变量明确选择尾帧/站位图/导演图时，才可写 @首帧、@站位图、@导演图；否则不要提。
2) 只保留可在画面中呈现的信息；真实台词、画外旁白、内心OS/吐槽只能放在 video 的台词/旁白段，不能放进 image。
3) image（首帧静帧出图词）：只写起幅静帧——主体(@角色)+场景(@场景)+构图/景别+光线/色调+情绪；不写运镜与动作过程，不写台词。
...
【台词/旁白】仅当有声音内容时写；对白为角色开口，画外旁白/内心OS/吐槽为画外或内心音轨，不出字幕
