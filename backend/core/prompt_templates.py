...
严格输出 JSON（不要任何额外文字），结构如下：
{
  ...
  "characters": [{"name":"角色名（多形态用 基名_形态）","trigger":"@","appearance":"按上述结构写全：性别、年龄数字、面部五官+肤质+妆容、发色+发质+发型+发饰、身材、服饰材质+颜色+纹样+款式、配饰","bio":"人物小传/简介：身份、经历、性格、与其他角色的关系、在本作中的叙事功能；若为某角色的形态则注明关系","note":"性格/戏剧功能；若为某角色的形态则注明关系"}],
...
衔接总原则（重要）：
- 「切镜头 / 景别切换」「承接上一镜尾帧」「站位图」「导演分镜信息图」是平权策略，没有任何一种是默认优先或强制兜底。
- 必须完全根据上一镜状态、本镜场景/动作/机位/衔接接口来判断哪种策略最自然、最利于防穿帮。
- 不要把「上一镜尾帧作为下一镜首帧」作为默认策略；只有当画面确实需要首尾连续，且尾帧能自然成为本镜起幅时才使用。
- 普通同场景对话、反打、动作推进可以选择切镜头，也可以选择站位图或导演图辅助，关键看空间关系和视觉调度是否需要。
- 本判断的重点是“让画面自然成立、剧情衔接清楚、减少穿帮”，而不是为了连续性强行相接。

仅输出 JSON（不要额外文字）：
{
  "scene_cut": true/false,            // 是否切换到全新时空（与上一镜不连续）
  "long_take": true/false,            // 本镜是否需要连续动作/长镜头式衔接
  "use_tail_frame": true/false,       // 是否把上一镜尾帧作为本镜首帧；只在尾帧天然适合作为起幅时 true
  "use_staging": true/false,          // 是否需要生成"站位图"统一人物位置/构图
  "use_director_board": true/false,   // 是否需要生成"导演分镜信息图"作为构图蓝本
  "shot_size": "本镜建议景别（远景/全景/中景/近景/特写），结合衔接需要判断",
  "camera_hint": "一句话运镜/景别指导：说明为什么采用当前衔接策略，以及镜头如何开始",
  "reason": "一句话说明判断依据"
}

判断准则：
- 切到新场景 / 时间跳跃 → 通常 scene_cut=true，use_tail_frame=false；可按需要使用导演图重新定调。
- 上一镜尾帧正好是本镜动作起点，且首尾连续能提升观感 → use_tail_frame=true。
- 需要保持人物方位、视线、左右关系，但不适合直接用尾帧起幅 → use_staging=true。
- 复杂调度、新段落开端、多人物/多道具关系或需要重新规划构图 → use_director_board=true。
- 普通反打、景别变化、情绪推进、同场景换角度 → 可选择切镜头，不必承接尾帧。
- 多种策略可以组合，但必须说明原因；不要机械套用尾帧策略。
